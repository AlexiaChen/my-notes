...
## Redis Cluster如何解决数据量大，高可用和高并发的问题
...
![[Pasted image 20240925135252.png]]

可以看到，Redis Cluster 整体的架构完全就是照抄 MySQL 构建集群的那一套东西（当然，这些设计和方法也不是 MySQL 发明的），抄作业抄的就差把名字一起也抄上了。

> 具体如何搭建 Redis Cluster 以及相关的操作命令你可以看一下[Scale with Redis Cluster | Docs](https://redis.io/docs/latest/operate/oss_and_stack/management/scaling/)
...
Redis Cluster 采用了一种去中心化的[流言 (Gossip) 协议](https://en.wikipedia.org/wiki/Gossip_protocol)来传播集群配置的变化。一般涉及到协议都比较复杂，这里我们不去深究具体协议和实现算法，我大概给你讲一下这个协议原理。
...
第三个作用就是维护集群的元数据，这个元数据主要就是集群所有节点的主从信息，以及槽和节点关系映射表。这个架构和我在  [[12-MySQL如何应对高并发02-读写分离]] 这节课中给你讲过的，用 HAProxy+Keepalived 来代理 MySQL 请求的架构是类似的，只是多了一个自动路由分片的功能而已。

![[Pasted image 20240925135645.png]]

像开源的 Redis 集群方案twemproxy [twitter/twemproxy: A fast, light-weight proxy for memcached and redis (github.com)](https://github.com/twitter/twemproxy)和Codis [CodisLabs/codis: Proxy based Redis cluster solution supporting pipeline and scaling dynamically (github.com)](https://github.com/CodisLabs/codis)，都是这种架构的。
...
![[Pasted image 20240925135804.png]]
