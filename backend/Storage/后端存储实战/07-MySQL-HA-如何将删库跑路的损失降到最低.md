...
所谓防患于未然，你从设计一个系统的第一天起，就需要考虑在出现各种问题的时候，如何来保证这个系统的数据安全性。今天我们来聊一聊，如何提前预防，将“删库跑路”等这类问题导致的损失尽量降到最低。如何更安全地做数据备份和恢复？
...
第二，在回放 Binlog 的时候，指定的起始时间可以比全量备份的时间稍微提前一点儿，确保全量备份之后的所有操作都在恢复的 Binlog 范围内，这样可以保证恢复的数据的完整性。因为回放 Binlog 的操作是具备幂等性的（为了确保回放幂等，需要设置 Binlog 的格式为 ROW 格式)
...
![[Pasted image 20240830180205.png]]


### QA

把binlog日志同步到从库，然后从库立即回放。那如果是删库的动作，从库也回放，不是把从库也干掉了。

> 所以回放的时候，要找准起止位点，不要回放删库动作。
