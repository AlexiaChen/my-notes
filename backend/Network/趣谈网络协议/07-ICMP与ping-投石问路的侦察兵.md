...
![[Pasted image 20240129142700.png]]
...
![[Pasted image 20240129143424.png]]
...
> 正常情况下，协议栈能正常走到udp，当然正常返回udp。 但是，你主机不可达，是ip层的（还没到udp）。ip层，当然只知道回icmp。报文分片错误也是同理。
...
>  网络不可达也是到达了某个地方，发现走不下去了，到哪里哪里返回，一般是某个路由器
