...
就像 TCP 协议一样。它之所以这么复杂，那是因为它秉承的是“性恶论”。它天然认为网络环境是恶劣的，丢包、乱序、重传，拥塞都是常有的事情，一言不合就可能送达不了，因而要从算法层面来保证可靠性。

### TCP包头格式

我们先来看 TCP 头的格式。从这个图上可以看出，它比 UDP 复杂得多。

![[Pasted image 20240201152131.png]]
...
另外，你作为服务端 B 的程序设计者，对于 A 这种长时间不发包的客户端，可以主动关闭，从而空出资源来给其他客户端使用。三次握手除了双方建立连接外，主要还是为了沟通一件事情，就是 TCP 包的序号的问题。
...
![[Pasted image 20240201152645.png]]
...
![[Pasted image 20240201152838.png]]
...
![[Pasted image 20240201153001.png]]
...
> 
> Connection reset错误说明，要连接的指定IP地址的机器不存在，或者指定IP地址的机器没有开启对应端口。解决办法是ping一下这个IP，如果ping不通，看下IP是否写错了或者该IP对应的机器是不是已经停机了，如果能ping通，看下该机器上的端口是不是没有开启
> 
> 一般如果有timewait了，那就意味着链接是曾经建立成功过的。但是，如果客户端出现大量timewait，那就意味着链接在建立之后很快就断开了，但是也很快重建了一个链接。这类情形在高并发，短请求的情况下会出现。如果数量不是特别多的话，怀疑客户端和服务端之间有其他的设备，比如4层或者7层的proxy设备，会截断TCP链接。这个时候如果阿里云的服务有问题，但是客户端和proxy设备间的连接一直没有数据的话，可能会超时断开。建议还是用tcpdump抓包看看问题到底出现在哪一段上了

> 流量控制和拥塞控制什么区别
> 一个是对另一端的，一个是针对网络的. 流量控制是根据对端能接收的空间大小、调整发送数据的多少；拥塞控制是根据TTL调整发送的快慢吧
...
> Tcp三次握手设计被恶意利用 ，造就了ddos。  是有连接攻击的
