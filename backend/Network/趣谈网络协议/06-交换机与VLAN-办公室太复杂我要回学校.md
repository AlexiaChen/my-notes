...
我们在宿舍里组建了一个本地的局域网 LAN，可以愉快地玩游戏了。这是一个非常简单的场景，因为只有一台交换机，电脑数目很少。今天，让我们切换到一个稍微复杂一点的场景，办公室。
...
![[Pasted image 20240126112832.png]]
...
![[Pasted image 20240126113421.png]]
...
![[Pasted image 20240126113901.png]]
...
![[Pasted image 20240126114313.png]]
...
![[Pasted image 20240126114411.png]]
...
![[Pasted image 20240126114502.png]]
...
![[Pasted image 20240126114540.png]]
...
![[Pasted image 20240126114640.png]]
...
![[Pasted image 20240126114705.png]]
...
![[Pasted image 20240126114855.png]]
...
![[Pasted image 20240126114944.png]]
...
> 第一张图中，机器三是如何同时链接两台交换机？   赞，我以为不会有人问这个问题的，哈哈，老的局域网都是连到线上的，所以延续了这个图，为了准确，这里面中间的局域网可以认为是一个非直连的，例如中间隐藏了交换机等细节的，为了说明这个理论而简化


> 所有交换机都支持STP协议吗？除了STP还有别的什么机制能防止或预防网络环路风暴？谢谢  有的，现在很少用stp了，后面讲数据中心的时候会提到

> 有一次办公室断网，排查时候发现路由器某一个部门的端口的灯在狂闪，拔掉后恢复正常。然后去那个部门排查才发现他们插错了口，形成了环路导致广播风暴。   是的，赞


> 看文章里最后一张图，两个不同的交换机，肯定是2个不同的网段，又为何能使用同一个vlanId进行网络划分呢？一个vlanId只能绑定一个网段才对吧？甚是不解   交换机可以识别vlan的，交换机本身没有所谓的网段，交换机的口上不配置ip.一个vlan一个网段是对的，在同一个vlan的机器配置相同的网段就可以了，和交换机关系不大


> 老师，你好~只用交换机连接起来的网络，都是在一个局域网里是吧？ 是的

> 既然stp能解决环路，为什么实际场景中还是有环路发生？ 不一定开启stp


> 请问这么多交换机都要分配id优先级？这个优先级是怎么标记的，保存在什么地方，如果有两个1怎么办？ 不能重复的，是管理员自己配置的，那个时候交换机数目没这么多


> 支持vlan的交换机之间是通过trunk口通信的，那一台交换机上trunk口有几个呢？如果只有一个，那就只能2台交换机连接在一起了，还是交换机上的所有网口都支持在普通和trunk之间随意切换？   每个口都可以配置的，不是死的，是可以通过命令进行配置