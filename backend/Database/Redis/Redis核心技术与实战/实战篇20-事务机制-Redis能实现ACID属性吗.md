...
而事务执行又可以分成命令入队（EXEC 命令执行前）和命令实际执行（EXEC 命令执行后）两个阶段，所以，我们就针对这两个阶段，分成两种情况来分析：
...
![[Pasted image 20240218100115.png]]
...
![[Pasted image 20240218100147.png]]
...
![[Pasted image 20240218100214.png]]
...
![[Pasted image 20240218100236.png]]
...
在执行事务时，如果 Redis 实例发生故障，而 Redis 使用的 RDB 机制，事务的原子性还能否得到保证？ 我觉得是可以保证原子性的。
...
> AOF如果开启always模式不是可以保证数据不丢失吗？为啥也保证不了持久性呢？  AOF是“写后”日志，如果刚写完还没来得及写日志就宕机了，就会丢失刚写的这一个数据。
