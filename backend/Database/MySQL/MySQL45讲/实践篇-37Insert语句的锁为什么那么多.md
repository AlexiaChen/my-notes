...
![[Pasted image 20230209105724.png]]
...
![[Pasted image 20230209105909.png]]

通过这个慢查询日志，我们看到 Rows_examined=1，正好验证了执行这条语句的扫描行数为 1。那么，如果我们是要把这样的一行数据插入到表 t 中的话：
...
![[Pasted image 20230209105943.png]]
...
![[Pasted image 20230209110010.png]]
...
所以，这个猜测不对。实际上，Explain 结果里的 rows=1 是因为受到了 limit 1 的影响。从另一个角度考虑的话，我们可以看看 InnoDB 扫描了多少行。如图 5 所示，是在执行这个语句前后查看 Innodb_rows_read 的结果。

![[Pasted image 20230209110037.png]]
...
![[Pasted image 20230209110219.png]]
...
![[Pasted image 20230209110326.png]]

在 session A 执行 rollback 语句回滚的时候，session C 几乎同时发现死锁并返回。

这个死锁产生的逻辑是这样的：

- 在 T1 时刻，启动 session A，并执行 insert 语句，此时在索引 c 的 c=5 上加了记录锁。注意，这个索引是唯一索引，因此退化为记录锁（如果你的印象模糊了，可以回顾下 [[MySQL45讲-实践篇-19为什么我只改一行语句-锁这么多]] 文章介绍的加锁规则）。
...
![[Pasted image 20230209110432.png]]
...
注意，如果有多个列违反了唯一性约束，就会按照索引的顺序，修改跟第一个索引冲突的行。现在表 t 里面已经有了 (1,1,1) 和 (2,2,2) 这两行，我们再来看看下面这个语句执行的效果：

![[Pasted image 20230209110528.png]]
