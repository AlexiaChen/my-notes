...
我在第 1 篇文章  [[MySQL45讲-基础篇-01基础架构#基础架构：一条SQL查询语句是如何执行的？]] 中说过，MySQL 建立连接的过程，成本是很高的。除了正常的网络连接三次握手外，还需要做登录权限判断和获得这个连接的数据读写权限。
...
![[Pasted image 20221216093945.png]]
...
但是，这种方法特别符合我们标题里说的“饮鸩止渴”，风险极高，是我特别不建议使用的方案。尤其你的库外网可访问的话，

就更不能这么做了。在 MySQL 8.0 版本里，如果你启用–skip-grant-tables 参数，MySQL 会默认把 --skip-networking 参数打开，表示这时候数据库只能被本地的客户端连接。可见，MySQL 官方对 skip-grant-tables 这个参数的安全问题也很重视。
...
比如，我们犯了在第 16 篇文章  [[MySQL45讲-实践篇-16为什么SQL语句逻辑相同但性能差异巨大]] 中提到的那些错误，导致语句没有使用上索引。
...
![[Pasted image 20221216094740.png]]

导致慢查询的第三种可能，就是碰上了我们在第 9 篇文章  [[MySQL45讲-实践篇-09有些时候会选错索引]] 中提到的情况，MySQL 选错了索引。
...
我以业务高峰期的性能问题为背景，和你介绍了一些紧急处理的手段。
