...
![[Pasted image 20231006212008.png]]
...
Kafka 的 Exactly Once 语义是通过它的事务和生产幂等两个特性来共同实现的。其中 Kafka 事务的实现原理，我们在  [[25-RocketMQ与Kafka中如何实现事务]] 这节课中讲过。它可以保证一个事务内的所有消息，要么都成功投递，要么都不投递。
...
所以，Flink 基于两阶段提交这个常用的分布式事务算法，实现了一分布式事务的控制器来解决这个问题。如果你对具体的实现原理感兴趣，可以看一下 Flink 官网文档中的[这篇文章]([An Overview of End-to-End Exactly-Once Processing in Apache Flink (with Apache Kafka, too!) | Apache Flink](https://flink.apache.org/2018/02/28/an-overview-of-end-to-end-exactly-once-processing-in-apache-flink-with-apache-kafka-too/))。
### Exactly Once版本的Web请求统计
...
![[Pasted image 20231006212528.png]]
...
完整代码请看 [liyue2008/kafka-flink-exactlyonce-example (github.com)](https://github.com/liyue2008/kafka-flink-exactlyonce-example)
## 小结
