...
然而有时候这个地址簿也经常给你指错路，明明距离你 500 米就有个吃饭的地方，非要把你推荐到 5 公里外。为什么会出现这样的情况呢？还记得吗？当我们发出请求解析 DNS 的时候，首先，会先连接到运营商本地的 DNS 服务器，由这个服务器帮我们去整棵 DNS 树上进行解析，然后将解析的结果返回给客户端。
...
![[Pasted image 20240204134440.png]]
...
![[Pasted image 20240204134620.png]]
...
![[Pasted image 20240204135355.png]]
...
![[Pasted image 20240204135514.png]]
...
![[Pasted image 20240204135627.png]]
...
![[Pasted image 20240204135711.png]]
...
![[Pasted image 20240204135730.png]]
...
![[Pasted image 20240204135836.png]]

### 小结

> HTTPNDS 其实就是，不走传统的 DNS 解析，而是自己搭建基于 HTTP 协议的 DNS 服务器集群，分布在多个地点和多个运营商。

> 这个文章看起来感觉httpdns能解决传统的dns问题，但是前提是有自己的客户端，这种要对业务进行全部改造。那么如果就是浏览器访问，有没有什么方案，在不使用cdn的情况下   HttpDNS需要自己的客户端

> HTTPDNS能支持HTTPS 不         可以

> 那httpdns不需要考虑负载均衡么?  考虑，httpdns里面也可以配置负载均衡策略的

> 请问老师：文中的在不同运营商或地区部署httpdns服务，根据客户端位置返回最优应用地址，httpdns服务端部署方式是如何的？如何达到不同运营商或地区效果？httpdns适用场景只有哪些呢？谢谢  一般是多个数据中心分布式部署，适用于手机客户端的情况

> 权威DNS服务器的ip地址不会变吗？   很少变
